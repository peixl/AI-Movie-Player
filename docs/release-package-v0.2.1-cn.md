...
```md
AI-Movie-Player 是一个面向本地片库的 AI 原生桌面电影播放器，目标是让元数据、字幕、推荐和电影对话都变得自然、优雅、真正有用。
...
现在已经加入：观影前 briefing、观影后复盘、双片连看建议，以及 Windows + macOS 双平台打包发布。
...
它仍然是一个本地片库电影播放器，但 AI 部分已经更进一步：不再只有 prompt 快捷入口，而是能生成结构化的观影工作流卡片，用于观影前导览、观影后复盘和双片连看建议。

这次我们也进一步整理了仓库首页和开源协作面，包括英文主文档、中文配套文档、Issue 模板、发布说明，以及更清晰的 Windows 与 macOS 双平台软件包交付路径。

目标依然没有变：把它做成一个安静高级、AI 原生、既让用户喜欢也让开发者尊重的电影播放器。
