use egui::{Color32, Context, pos2, Rect, RichText, Rounding, Sense, Stroke, Ui, Vec2};
...
impl AppLayout {
    ...
    pub fn show_sidebar(&mut self, ui: &mut Ui, ctx: &Context, is_dark: bool, movie_count: i64) -> bool {
        ...
        for (view, icon, label, badge) in &nav_items {
            ...
            let bg = if is_active { primary } else { Color32::TRANSPARENT };
            ...
            if ui.is_rect_visible(rect) {
                ...
                let hover_t = ctx.animate_bool(
                    egui::Id::new(format!("nav_hover_{}", label)),
                    response.hovered(),
                );
        ...
        for (view, icon, label, badge) in &ai_nav_items {
            ...
            if ui.is_rect_visible(rect) {
                ...
                // Badge
                if !badge.is_empty() && !is_active {
                    let badge_color = if *view == View::AiChat || *view == View::AiRecommend {
                        primary.linear_multiply(0.8)
                    } else {
                        dim
                    };
                    let badge_galley = ui.painter().layout_no_wrap(
                    ...
                    let badge_rect = egui::Rect::from_min_size(
                        pos2(rect.max.x - badge_galley.size().x - 16.0, rect.center().y - badge_galley.size().y / 2.0 - 2.0),
                        badge_galley.size() + egui::vec2(8.0, 4.0),
                    );
                    ui.painter().rect_filled(badge_rect, Rounding::same(4.0), badge_bg);
                    ui.painter().galley(
                        pos2(badge_rect.min.x + 4.0, badge_rect.center().y - badge_galley.size().y / 2.0),
                        badge_galley,
                        badge_color,
                    );
                }
    ...
    pub fn show_content(&self, _ui: &mut Ui) {}
}

/// Draw a hand-drawn icon at a specific screen position (used in custom layout)
fn draw_icon_at(painter: &egui::Painter, name: &str, size: f32, center: egui::Pos2, color: Color32) {
...
fn wobble(x: f32, y: f32, seed: f32) -> egui::Pos2 {
...
fn draw_film_icon(painter: &egui::Painter, center: egui::Pos2, size: f32, color: Color32, seed: f32) {
    let r = size / 2.0 - 2.0;
    ...
    for i in 0..pts.len() {
        let p1 = pts[i]; let p2 = pts[(i+1)%pts.len()];
        let mid = pos2((p1.x+p2.x)/2.0, (p1.y+p2.y)/2.0);
        painter.add(egui::Shape::CubicBezier(egui::epaint::CubicBezierShape {
            points: [p1, mid, mid, p2], closed: false,
            fill: Color32::TRANSPARENT, stroke,
        }));
    }
    // Sprocket holes
    for i in 0..3 {
        let y = rect.min.y + rect.height()*(i as f32+0.5)/3.0;
        painter.circle_filled(wobble(rect.min.x+3.0, y, seed+100.0+i as f32), 1.5, color.linear_multiply(0.5));
        painter.circle_filled(wobble(rect.max.x-3.0, y, seed+200.0+i as f32), 1.5, color.linear_multiply(0.5));
    }
    // Play triangle
    let tri = [
...
fn draw_add_folder_icon(painter: &egui::Painter, center: egui::Pos2, size: f32, color: Color32, seed: f32) {
    let fw = size * 0.55; let fh = size * 0.4;
    let l = center.x - fw/2.0; let t = center.y - fh/2.0 + 3.0;
    // Tab
    let tw = fw * 0.3; let th = fh * 0.2;
    let tab = [
        wobble(l, t+th, seed), wobble(l+tw*0.3, t+th, seed+1.0),
        wobble(l+tw*0.5, t, seed+2.0), wobble(l+tw, t, seed+3.0),
        wobble(l+tw, t+th, seed+4.0),
    ];
    painter.add(egui::Shape::convex_polygon(tab.to_vec(), color.linear_multiply(0.12), Stroke::new(1.2, color)));
    // Body rect
    let body = Rect::from_min_max(pos2(l, t+th*0.3), pos2(center.x+fw/2.0, center.y+fh/2.0));
    ...
    // Plus
    let cx = center.x + 1.0; let cy = center.y + 4.0; let arm = 4.0;
...
fn draw_subtitle_icon(painter: &egui::Painter, center: egui::Pos2, size: f32, color: Color32, seed: f32) {
    let bw = size * 0.5; let bh = size * 0.38;
    ...
    // Tail
    let tail = [
        wobble(center.x-3.0, r.max.y, seed), wobble(center.x+1.0, r.max.y+6.0, seed+1.0),
        wobble(center.x+5.0, r.max.y, seed+2.0),
    ];
    painter.add(egui::Shape::convex_polygon(tail.to_vec(), color.linear_multiply(0.06), Stroke::new(1.2, color)));
    // Text lines
    for i in 0..2 {
        ...
        painter.add(egui::Shape::line_segment([p1,p2], Stroke::new(1.0, color.linear_multiply(0.5))));
    }
}

fn draw_bolt_icon(painter: &egui::Painter, center: egui::Pos2, size: f32, color: Color32, seed: f32) {
    let pts = [
        wobble(center.x+1.0, center.y-7.0, seed), wobble(center.x-3.0, center.y-1.0, seed+1.0),
        wobble(center.x-1.0, center.y-1.0, seed+2.0), wobble(center.x-2.0, center.y+7.0, seed+3.0),
        wobble(center.x+2.0, center.y+1.0, seed+4.0), wobble(center.x, center.y+1.0, seed+5.0),
    ];
    let fill = color.linear_multiply(0.18);
    for i in 0..pts.len()-2 {
        painter.add(egui::Shape::convex_polygon([pts[0], pts[i+1], pts[i+2]].to_vec(), fill, Stroke::NONE));
    }
...
fn draw_bookmark_icon(painter: &egui::Painter, center: egui::Pos2, size: f32, color: Color32, seed: f32) {
    let w = size * 0.32; let h = size * 0.5;
    let pts = [
        wobble(center.x-w, center.y-h, seed), wobble(center.x+w, center.y-h, seed+1.0),
        wobble(center.x+w, center.y+h*0.7, seed+2.0), wobble(center.x, center.y+h*0.2, seed+3.0),
        wobble(center.x-w, center.y+h*0.7, seed+4.0),
    ];
    for i in 0..pts.len() {
        painter.add(egui::Shape::line_segment([pts[i], pts[(i+1)%pts.len()]], Stroke::new(1.5, color)));
    }
    painter.circle_filled(pos2(center.x, center.y+h*0.25), w*0.7, color.linear_multiply(0.06));
    // Checkmark
    let ck = [
        wobble(center.x-3.0, center.y, seed+50.0), wobble(center.x-0.5, center.y+2.5, seed+51.0),
...
fn draw_gear_icon(painter: &egui::Painter, center: egui::Pos2, size: f32, color: Color32, seed: f32) {
    let or = size * 0.25; let ir = size * 0.1;
    // Teeth
    for i in 0..8 {
        let a = i as f32 * std::f32::consts::TAU / 8.0;
        let p1 = pos2(center.x + a.cos()*(or-0.5), center.y + a.sin()*(or-0.5));
        let p2 = pos2(center.x + a.cos()*(or+2.0), center.y + a.sin()*(or+2.0));
        painter.add(egui::Shape::line_segment([wobble(p1.x,p1.y,seed+i as f32), wobble(p2.x,p2.y,seed+10.0+i as f32)], Stroke::new(1.5, color)));
    }
    // Outer circle
    let cpts: Vec<egui::Pos2> = (0..14).map(|i| {
        let a = i as f32 * std::f32::consts::TAU/14.0;
        wobble(center.x+a.cos()*or, center.y+a.sin()*or, seed+100.0+i as f32)
    }).collect();
    for i in 0..cpts.len() {
        let p1=cpts[i]; let p2=cpts[(i+1)%cpts.len()]; let m=pos2((p1.x+p2.x)/2.0,(p1.y+p2.y)/2.0);
        painter.add(egui::Shape::CubicBezier(egui::epaint::CubicBezierShape{points:[p1,m,m,p2],closed:false,fill:Color32::TRANSPARENT,stroke:Stroke::new(1.2,color)}));
    }
    painter.circle_filled(center, ir, color.linear_multiply(0.08));
    painter.circle_stroke(center, ir, Stroke::new(1.2, color));
}

fn draw_search_icon(painter: &egui::Painter, center: egui::Pos2, size: f32, color: Color32, seed: f32) {
    let gr = size * 0.24;
    let gc = center - egui::vec2(1.5, 1.5);
    let cpts: Vec<egui::Pos2> = (0..12).map(|i| {
        let a = i as f32 * std::f32::consts::TAU/12.0;
        wobble(gc.x+a.cos()*gr, gc.y+a.sin()*gr, seed+i as f32)
    }).collect();
    for i in 0..cpts.len() {
        let p1=cpts[i]; let p2=cpts[(i+1)%cpts.len()]; let m=pos2((p1.x+p2.x)/2.0,(p1.y+p2.y)/2.0);
        painter.add(egui::Shape::CubicBezier(egui::epaint::CubicBezierShape{points:[p1,m,m,p2],closed:false,fill:Color32::TRANSPARENT,stroke:Stroke::new(1.5,color)}));
    }
...
fn draw_chat_icon(painter: &egui::Painter, center: egui::Pos2, size: f32, color: Color32, seed: f32) {
    let bw = size * 0.5;
    let bh = size * 0.38;
    let r = Rect::from_center_size(center - egui::vec2(0.0, 1.0), egui::vec2(bw, bh));
    painter.rect_filled(r, Rounding::same(5.0), color.linear_multiply(0.06));
...
fn draw_sparkle_icon(painter: &egui::Painter, center: egui::Pos2, size: f32, color: Color32, seed: f32) {
    let r = size * 0.38;
    for i in 0..4 {
        ...
        let p_inner1 = wobble(
            center.x + a2.cos() * r * 0.3, center.y + a2.sin() * r * 0.3,
            seed + 10.0 + i as f32,
        );
        let p_inner2 = wobble(
        ...
        let p_next = wobble(center.x + next_a.cos() * r, center.y + next_a.sin() * r, seed + (i + 1) as f32);
