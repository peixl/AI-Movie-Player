...
impl BatchOpsPanel {
    ...
    pub fn show(&mut self, ui: &mut Ui, is_dark: bool) {
        ...
        if !self.scanning {
            ...
            ui.add_space(16.0);
            if ui.button(RichText::new("Start Batch Scan / 开始批量扫描").size(14.0).fill(Color32::WHITE)).clicked() {
                self.scanning = true;
                self.message = Some("请选择要扫描的文件夹 / Select a folder to scan".into());
            }
        } else {
            ui.label(format!("扫描中：已处理 {} / {} 个文件 / Scanning: {} of {} files", self.processed, self.total_files, self.processed, self.total_files));
        }
        ...
        if self.scanning && self.total_files > 0 {
            ...
            ui.add_space(8.0);
            ui.label(format!("已导入 {} | 已跳过 {} | 错误 {} / Imported {} | Skipped {} | Errors {}",
                self.imported_count, self.skipped_count, self.failed_count,
                self.imported_count, self.skipped_count, self.failed_count));
            if let Some(ref file) = self.current_file {
                ui.label(RichText::new(format!("当前文件 / Current file: {}", file)).size(12.0).color(dim));
            }
        }
